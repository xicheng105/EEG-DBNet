...
def load_data_BCI2a(data_path, subject, training):
    """
    Loading and dividing of the data based on the subject-specific (subject-dependent) approach.

    In this approach, we used the same training and testing data as the original competition, i.e., 288 * 9 trials in
    session 1 for training, and 288 * 9 trials in session 2 for testing.

    :param data_path: string
        Path to the data folder.
        Dataset BCI Competition IV-2a is available on https://bnci-horizon-2020.eu/database/data-sets
    :param subject: int
        Number of a subject in [1, 2, ..., 9].
        The subject data is used to test the model and other subject data for training
    :param training: bool
        If True, load training data
        If False, load testing data
    :return:
        data_return, numpy matrix, size = n_valid_trial * n_channels * window_length
        class_return, numpy matrix, size = n_valid_trial
    """
...
def load_data_loso(data_path, subject, DataSet='BCI2a'):
    ...
    :param subject: int
        ...
        The subject data is used to test the model and other subject data for training
    :return:
        X_train, numpy matrix, size = n_valid_trial * n_channels * window_length
        y_train, numpy matrix, size = n_valid_trial
        X_test, numpy matrix, size = n_valid_trial * n_channels * window_length
        y_test, numpy matrix, size = n_valid_trial
    """
...
def get_data(data_path, subject, LOSO=False, isStandard=True, freFilter=False, DataSet='BCI2a'):
    if DataSet == 'BCI2a':
        ...
        if LOSO:
            ...
            # Loading and Dividing of the data set based on the subject-specific (subject-dependent) approach. In this
            # approach, we used the same training and testing data as the original competition, i.e., trials in session 1
            # for training, and trials in session 2 for testing.
    ...
    # Convert the labels from a numbering starting from 1 to a numbering starting from 0.
    # Make sure the type of label is int.
